...
void drawPinwheel(float dTheta, float r, float& theta, int* p, int numSegments, ConsoleEngine* engine) {
	theta += dTheta;
	int color = 0;
...
int notmain() {
	...
	while (true) {
		...
		if (1) {
			...
			drawPinwheel(0, 70, theta, p, 1000, &engine);
			drawPinwheel(dTheta, 40, theta, p, 150, &engine);
			engine.outputScreen();
